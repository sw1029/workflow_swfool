...
If a hook is absent, fail quiet and keep existing schema-contract rules, except Q5 hook-contract absence is visible debt only: record debt/warn evidence when a consumer attempted to use the missing hook, keep the underlying hook-dependent judgment as no-op, and do not force implementation from this skill. Do not infer privacy classes, identifier forms, merge depth, field authority, formulas, sampled populations, hook existence, hook debt thresholds, or code-convention policy from project filenames, sample text, or domain vocabulary in this skill.
