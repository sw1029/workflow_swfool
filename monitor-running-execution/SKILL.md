...
7. Append through the stage-observer producer as `step: run` with `event_kind: compiled_stage_observation_ref` and `observation_kind: long_run_monitor` (or the caller's bounded subtype); do not use public append or create a noncanonical `monitor` phase.
