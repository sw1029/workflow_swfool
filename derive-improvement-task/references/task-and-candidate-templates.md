...
3. Preserve the candidate as immutable selection history. Record its applied or superseded identity transition, its link to the selected task identity, and the committed selection-publication receipt; keep the mutable current selection pointer separate.
4. Mention the retained candidate binding and current selected-task pointer in the final response and `past_task` log note. Physical retention cleanup, if ever required, is a separate authorized operation and cannot reactivate or rewrite completed history.

If no previous `task.md` exists, treat the write as `initial_init`: skip `past_task` logging, write the initial `task.md`, index it when possible, and retain the applied candidate after the new task and candidate transition are recorded.
