...
- When the caller supplies `effective_allowed_dispositions`, choose the selected task disposition from that list only. Treat gate `allowed_dispositions` as an intersection already computed by loopback/orchestration, not a menu to union. If the only effective options are `terminal_blocked` and `user_escalation`, do not write another standalone consolidation task.
...
- If `anti_loop_progress_gate.structure_metrics_gate.structure_consolidation_recommended=true`, Class C consolidation or module-boundary work is valid only when it reduces the reported structure burden or unlocks the next primary-output transition.
...
- Do not seal or terminal-block a blocker family when `untried_actionable_root_cause_exists=true`. Promote the first untried local, bounded, provider-free, in-scope, authority-allowed hypothesis from `untried_root_cause_hypotheses` as the next `goal_productive` repair task.
...
1. Load planning context.
   ...
   - Read any caller-supplied `loop_breaker_packet`, including blocker signature repetition, `semantic_signature` repetition, sealed-family matches, goal-distance gate, governance-only streak, positive input delta gate with `has_supplied_input_delta` and `supplied_input_artifact_paths`, provider re-attempt gate, command-surface budget, and terminal blocker recommendation.
   - Read any caller-supplied `output_delta_packet`, `qualitative_review_packet`, and `anti_loop_progress_gate`. Use `changed_vs_previous`, `semantic_progress`, semantic readiness/cap fields, provider/env behavior booleans, `previous_accepted_baseline`, `coverage_quality_delta_gate`, `coverage_quality_delta_reconciliation_gate`, `structure_metrics_gate`, `provider_scale_dispatch_gate`, measurement-progress fields, blocker-mutation fields, and same-family micro-hardening counts as hard selection constraints, not optional commentary.
   - Read `effective_allowed_dispositions`, `disposition_intersection_basis`, `consolidation_streak`, `measurement_progress_allowed`, `measurement_streak`, `measurement_progress_streak_for_root_key`, `measurement_progress_streak_for_root_family`, `root_family_key`, `blocker_root_family`, `task_correction_class`, `detection_only_streak_for_root_family`, `requires_correction_or_terminal`, `validator_integrity_gate`, `blocker_mutation_kind`, `terminal_outcome_changed`, `forward_mutation_vacuous`, `forward_mutation_budget_remaining`, `root_cause_ledger_path`, `untried_actionable_root_cause_exists`, `untried_root_cause_hypotheses`, `force_implementation_cycle`, `authoritative_semantic_progress`, R-GCOV findings, and `validator_disagreement` findings when present. Treat them as hard selection constraints.
...
8. Synthesize the final task with one agent.
   ...
   - If `untried_actionable_root_cause_exists=true`, it must not terminal-block or seal the family. It must select the untried hypothesis as the next repair task and include `root_cause_ledger_path` plus the chosen `hypothesized_root_cause` in the derive result.
