...
- If `anti_loop_progress_gate.cumulative_goal_distance_stalled=true`, select only `terminal_blocked` or `user_escalation` unless `adapter_mandate_required=true` is the preceding forced adapter task. If `untried_veto_overridden_by_chain_stall=true`, do not promote another distinct untried hypothesis merely because its label or terminal outcome is new.
