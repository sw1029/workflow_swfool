...
- Before using migration evidence as a governed trust-boundary pass, run the source-separated `scripts/agent_log_migration_verifier.py` described in [legacy-migration.md](references/legacy-migration.md). Producer/self-declared success and the producer's own `validate` result are not independent verification.
