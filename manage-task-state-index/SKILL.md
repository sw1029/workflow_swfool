...
6. Run global ID consistency audit.
   ...
   - `audit` computes the full global audit but prints a bounded summary by default, including every current-surface blocker and historical-debt counts by severity. `--summary-only` remains an explicit compatibility spelling; add `--focus-path <path>` for focused issues or `--full-output` when the complete historical issue list is required.
   - Summary output is a display projection, not a replacement for `--full-output` or `--write-report` evidence when deletion, issue closure, or another destructive transition depends on global consistency.
...
- `audit`: inspect global ID consistency, broken links, duplicate active paths, stale digests, missing files, active task conflicts, and unindexed standard artifacts. It prints bounded counts, all current blockers, and focused issues by default; use `--full-output` for the complete historical list. `--summary-only` remains accepted. Without `--write-report`, it shares an existing lock or verifies a stable lockless snapshot and creates no `.task` or lock. Add `--write-report` to write the full `.task/id_audit/*.md` evidence and index it as `audit-*`.
