...
- `root_family_key`
- `artifact_family`
- `decision_artifact_ref`: exact decision input identity containing opaque artifact id, class, SHA-256, production lane, path/store reference, discovery basis, and scope-verification status.
...
- `input_state_fingerprint` and `attempt_identity`: content-bound registry identity over cycle id, canonical artifact family, blocker signature, artifact hash/lane, and caller input fingerprints. Presentation labels are not identity inputs.
- `semantic_signature`
- `changed_vs_previous`
- `semantic_progress`
...
- `portfolio_quota_gate`, `recent_verifier_like_count`, `recent_producer_like_count`, `portfolio_ratio`, `portfolio_quota_exceeded`, `portfolio_quota_mode`, and `allowed_next_work_kinds`: L4 fields. Adapter-supplied quotas may restrict next selection; default quota evidence is warn-only.
...
Consumer context rule: `consumer_context_conformance` rows are actual invocation receipts keyed by project-owned required consumer IDs or a decision-required hook that was invoked. Each required row includes `consumer_context_id`, `adapter_loaded`, `hook_resolved`, `required_hook_callable`, `hook_signature_compatible`, `invocation_completed`, `return_contract_valid`, `artifact_identity_echo_valid`, `value_consumed_by_decision`, and `probe_evidence_id`. Optional absent hooks are not global failures. Repository-root import, hook strings, metric labels, or adapter self-attestation do not satisfy a row.

Progress ownership rule: run/review packets provide observation claims. This packet uniquely owns pre-derive `authoritative_semantic_progress`; completion validation uniquely owns `authoritative_progress_verdict`. Downstream packets may only preserve or downgrade these values. Terminal-outcome change is positive only with independent evidence, no conflict, and all required integrity axes evaluated.
