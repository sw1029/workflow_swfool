...
Consumer context rule: `consumer_context_conformance` rows are external loader probes keyed by project-owned required consumer IDs. Each required row includes `consumer_context_id`, `adapter_loaded`, `required_hook_callable`, `hook_signature_compatible`, `return_contract_valid`, and `probe_evidence_id`. Repository-root import, hook strings, metric labels, or adapter self-attestation do not satisfy a row.
