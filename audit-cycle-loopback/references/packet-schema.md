...
- `root_family_key`
- `artifact_family`
- `decision_artifact_ref`: exact decision input identity containing opaque artifact id, class, SHA-256, production lane, path/store reference, discovery basis, and scope-verification status. Any artifact-body, semantic, primary-metric, or goal claim additionally requires the full current `body_projection_fingerprint` and a non-empty cohort under `verification_input_ids` or `input_fingerprints`.
...
- `verification_source_separation_gate`: optional H4 packet classifying each axis as source-disjoint, overlapping, unknown, or `self_grounded`. Self-grounded structural checks are not source-independent semantic verification.
...
- `primary_metric_gate`, `primary_metric_high_water_moved`, `primary_metric_zero_movement_streak`, and `primary_metric_stalled`: D4/C4 trigger-key fields when the adapter exposes `primary_metric(...)`. The adapter supplies the stable goal-axis/metric id; the generic reducer compares only exact-bound, source-separated values under that stable scope. Local G-CHAIN family recurrence remains a separate counter. Secondary safety, artifact-truth, structure, storage, or diagnostic movement does not reset the primary high-water.
...
- `verification_source_separation_gate`, `verification_input_ids`, `producer_input_ids`, `verified_artifact_ids`, `input_fingerprints`, `verification_axes`, `self_grounded_fields`, `independent_source_separation_status`, and `independently_verified_downgraded_fields`: H4/A4 fields. Missing, overlapping, or self-grounded verification downgrades affected fields from source-independent semantic evidence while preserving the axis provenance.
...
- `cycle_reachability_gate`, `acceptance_scale`, `throughput_evidence`, `required_scale`, `observed_cycle_throughput`, `cycle_execution_cap`, `unreachable_within_cycle`, `long_run_launch_required`, and `harvest_validation_required`: L5 fields. They route cycle-unreachable targets to long-run launch/monitor/harvest, throughput improvement, descope, terminal blocker, or escalation.
...
Verification source separation rule: consume `independently_verified_fields` only when the axis receipt proves disjoint verification and producer identities/fingerprints. Preserve `self_grounded` as structural provenance, but do not promote it to source-independent semantic evidence. If source separation is missing, overlapping, blocked, unknown, or self-grounded, do not use downgraded fields for high-water movement, close, or `goal_productive`.
...
Prepared-mutation rule: `registry_updated=false` is invariant for loopback evaluation. `registry_update_candidate`, `registry_update_status`, `write_registry_deferred`, and `durable_mutation_candidate` expose replayable `typed_operations` for the existing registry, ledger, and seal. Each operation contains `operation_type`, opaque `target_id`, a complete projection `payload`, and `payload_sha256`; hashes without payloads are invalid. The candidate also includes a content hash, attempt identity, proposed revision, optional supersession, and finalization owner. Hash verification removes `candidate_sha256` before recomputing the candidate hash. `--write-registry` only sets `legacy_write_requested=true`; it never performs a write. A later evaluation may consume a finalized projection only by calling `cycle_ledger.load_current_finalized_state(root, finalized_cycle_id)`; never trust an unverified packet copy or select a prior cycle by mtime.
