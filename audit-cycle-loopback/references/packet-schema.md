...
- `input_state_fingerprint` and `attempt_identity`: version-2 content-bound registry identity over cycle id plus the exact artifact hash/lane and caller input fingerprints captured by `input_state_fingerprint`. Family, root, blocker, and hypothesis labels are trace metadata, not identity inputs.
