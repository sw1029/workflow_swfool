    ...
    if isinstance(value, dict):
        for key, child in value.items():
            if str(key).strip().lower() in keys:
                scalars = _domain.scalar_strings(child)
                if scalars:
                    return scalars[0]
...
def terminal_outcome_changed(output_delta: Any, changed_vs_previous: bool, semantic_progress: bool) -> bool:
    ...
    strict_changed = _values.bool_value(changed) if changed is not None else changed_vs_previous
    strict_semantic = _values.bool_value(semantic) if semantic is not None else semantic_progress
    if _values.bool_value(metadata):
        return False
    ...
    if produced is not None and not _values.bool_value(produced):
        return False
    return _values.bool_value(produced) and strict_changed and strict_semantic
...
def terminal_self_resolution_gate(*values: Any) -> dict[str, Any]:
    ...
    for value in values:
        for row in _values.iter_dicts(value):
            terminal_requested = terminal_requested or any(
                _values.bool_value(row.get(key)) for key in ("terminal_justified", "terminal_requested", "user_escalation_required")
            )
            terminal_requested = terminal_requested or any(
...
def terminal_outcome_key(output_delta: Any, changed_vs_previous: bool, semantic_progress: bool) -> str:
    ...
    elif produced is not None and not _values.bool_value(produced):
        base = "no_primary_output_delta"
    elif observed and observed not in {"unknown", "none", "null"}:
        base = observed
    else:
        base = "no_semantic_output_delta"
    return _families.normalize_root_family_key(base, status or "")
...
def terminal_outcome_root_family(
    ...
    *,
    ...
    if facet_map:
        mapped = _domain.collapse_root_family(facet_map, root_key, semantic_signature, artifact_family, outcome_key)
        return mapped, "facet_root_map", False
    return _families.normalize_root_family_key(artifact_family, outcome_key), "terminal_outcome_fallback", True

def latest_root_family_row(rows: list[dict[str, Any]], root_family_key: str) -> dict[str, Any] | None:
    return next((row for row in reversed(rows) if _measurement.row_root_family(row) == root_family_key), None)

def row_effective_count_key(row: dict[str, Any]) -> str:
    return str(row.get("failure_surface_count_key") or row.get("effective_count_key") or _measurement.row_root_family(row))

def previous_micro_hardening_count(rows: list[dict[str, Any]], root_family_key: str) -> int:
    family_rows = [row for row in rows if _measurement.row_root_family(row) == root_family_key]
    if not family_rows:
        return 0
    return max(_values.int_metric(row.get("same_family_micro_hardening_count") or row.get("micro_hardening_count") or 0) for row in family_rows)
...
def normalize_root_cause_slug(value: Any) -> str:
    return _families.normalize_root_family_key(str(value or "unknown_root_cause"))
