"""Public facade for conservative loopback evaluation."""
...
_DOMAIN_ADAPTER_MODULE: _TypingAny | None = None


def _push_runtime_cache() -> None:
    _adapters._set_cached_domain_adapter(_DOMAIN_ADAPTER_MODULE)


def _pull_runtime_cache() -> None:
    global _DOMAIN_ADAPTER_MODULE
    _DOMAIN_ADAPTER_MODULE = _adapters._cached_domain_adapter()


def evaluate(
    args: _TypingAny,
) -> tuple[dict[str, _TypingAny], list[dict[str, _TypingAny]], bool]:
    _push_runtime_cache()
    try:
        return _evaluator.evaluate(args)
    finally:
        _pull_runtime_cache()
