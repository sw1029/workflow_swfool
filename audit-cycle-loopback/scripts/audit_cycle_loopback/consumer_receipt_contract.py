"""Self-contained consumer receipt binding for standalone loopback audits."""
...
def _declared_id_list(row: dict[str, Any], field: str) -> tuple[list[str], bool]:
    value = row.get(field)
    if field not in row or not isinstance(value, list):
        return [], False
    normalized = [
        item.strip() for item in value if isinstance(item, str) and item.strip()
    ]
    valid = len(normalized) == len(value) and len(set(normalized)) == len(normalized)
...
def _expected_id_list(value: Any) -> tuple[list[str] | None, bool]:
    ...
    valid = len(normalized) == len(value) and len(set(normalized)) == len(normalized)
    return sorted(normalized), valid
...
def _local_coverage_status(row: dict[str, Any]) -> str:
    ...
    if consumed_hooks != required_hooks or consumed_gates & excluded_gates:
        return "not_evaluated"
...
def validate_consumer_receipt_binding(
    ...
    *,
    ...
    parsed = {field: _declared_id_list(row, field) for field in COVERAGE_FIELDS}
    mismatches.extend(field for field, (_, valid) in parsed.items() if not valid)
    required_hooks = set(parsed["required_hook_ids"][0])
    required_gates = set(parsed["required_gate_ids"][0])
    consumed_hooks = set(parsed["consumed_hook_ids"][0])
    consumed_gates = set(parsed["consumed_gate_ids"][0])
    ...
    if str(row.get("result_contract_status") or "").strip().lower() != status:
...
def consumer_receipt_binding_basis(row: dict[str, Any]) -> dict[str, Any]:
    """Return the canonical cross-owner consumer-receipt hash basis."""
    basis: dict[str, Any] = {
        ...
        "verification_input_ids": sorted(
            str(item)
            for item in _vectors.string_list(row.get("verification_input_ids"))
        ),
