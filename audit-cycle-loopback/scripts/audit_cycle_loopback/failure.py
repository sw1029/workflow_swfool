...
def normalize_classification_stage_map(value: Any) -> dict[str, set[str]]:
    ...
    def add(classification: Any, stages: Any) -> None:
        key = _families.normalize_root_family_key(classification)
        if not key:
            return
        stage_values = _vectors.string_list(stages)
        if isinstance(stages, dict):
            for child_key in ("stages", "failure_stages", "allowed_failure_stages", "stage"):
                stage_values.extend(_vectors.string_list(stages.get(child_key)))
...
def terminal_stage_resolution_gate(
    *,
    ...
    last_stage = normalize_stage_name(_values.first_field_value(contexts, LAST_STAGE_KEYS))
    failure_stage = normalize_stage_name(_values.first_field_value(contexts, FAILURE_STAGE_KEYS)) or next_failure_surface_stage(stages, last_stage)
    terminal_classification = _values.first_field_value(contexts, TERMINAL_CLASSIFICATION_KEYS)
    terminal_key = _families.normalize_root_family_key(terminal_classification) if terminal_classification is not None else None
    ...
    failure_surface_count_key = _families.normalize_root_family_key(root_family_key, dominant_parameter, failure_stage) if failure_stage else None
    return {
...
def same_input_contract_gate(contexts: list[Any]) -> dict[str, Any]:
    for value in contexts:
        for item in _values.iter_dicts(value):
            match_value = (
            ...
            if not declared:
                continue
            mismatch = (match_value is not None and not _values.bool_value(match_value))
...
def diagnostics_unavailable_gate(
    *,
    ...
    diagnostics_unavailable = any(_values.bool_value(_values.first_field_value([context], {"diagnostics_unavailable"})) for context in contexts)
    ...
    if diagnostics_unavailable and failure_surface_count_key:
        for row in reversed(registry_rows):
            if row.get("failure_surface_count_key") != failure_surface_count_key:
                continue
            if _values.bool_value(row.get("diagnostics_unavailable")):
                streak += 1
                continue
            break
    ...
    threshold_value = _values.budget_value(budget_contract)
