...
def coverage_gate_pass_value(gate: dict[str, Any]) -> bool | None:
    ...
    if "quality_delta_pass" in gate:
        return _values.bool_value(gate.get("quality_delta_pass"))
...
def compact_coverage_gate(gate: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "improved_fields": _values.list_values(gate.get("improved_fields")),
        "not_applicable_fields": _values.list_values(gate.get("not_applicable_fields")),
        "insufficient_evidence_fields": _values.list_values(gate.get("insufficient_evidence_fields")),
        "invalid_contract_fields": _values.list_values(gate.get("invalid_contract_fields")),
...
def structure_metrics_gate(value: Any) -> dict[str, Any]:
    ...
    recommended = any(
        _values.bool_value(metrics.get(key))
        for key in (
    ...
    high_water_moved = _values.bool_value(
    ...
    global_high_water_moved = _values.bool_value(
    ...
    refactor_effect_required = _values.bool_value(
...
def normalize_evidence_provenance(value: Any) -> tuple[dict[str, str], bool]:
    ...
    def add(metric_key: Any, provenance_value: Any) -> None:
        key = _verification.normalize_gate_key(metric_key)
...
def provenance_for_metric(metric_key: str, provenance: dict[str, str], hook_provided: bool) -> str:
    ...
    return provenance.get(_verification.normalize_gate_key(metric_key), "producer_attested")
...
def apply_evidence_provenance_filter(
    ...
    *,
    ...
    if not hook_provided:
        return gate, [], [], []
    updated = dict(gate)
    improved = _values.list_values(updated.get(improved_key))
