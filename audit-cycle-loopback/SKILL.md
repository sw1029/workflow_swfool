...
- `structure_metrics(...) -> dict`: optional entrypoint/module structure metrics such as LOC, command count, function count, consolidation recommendation, `structure_high_water_moved`, `improved_structure_axes`, and `refactor_effect_required` for S-STRUCT.
...
- Preserve `structure_metrics_gate.structure_high_water_moved`, `improved_structure_axes`, and `refactor_effect_required` when the adapter supplies them. Downstream validation uses these fields to prevent behavior-preserving refactor tasks from closing on module creation plus green tests when the original objective required structural reduction.
