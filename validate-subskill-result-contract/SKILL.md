...
- Treat consumer-context conformance as an external invocation receipt. Required rows must prove adapter load, hook resolution/callability, signature bind, invocation completion, return validity, exact artifact echo, decision consumption, and a bounded probe evidence ref; import or self-attestation is not wiring.
...
- Preserve axis-scoped verification provenance. `self_grounded` can satisfy structural recomputation but never source-independent semantic evidence or high-water movement.
