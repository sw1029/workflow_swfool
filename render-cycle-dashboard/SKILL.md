...
2. Derive phase status, task IDs, validation/progress verdicts, blockers, changed files, evidence paths, issue state, and commit results only from structurally valid, cycle-matching, non-duplicate events. Keep rejected events visible as diagnostics, but never consume their truth fields.
   - For compact format-v2 stage rows, display the scalar envelope immediately and hydrate the exact packet-CAS result only when a field is needed. Verify containment, size, raw SHA-256, and canonical-body SHA-256 first; show a blocking hydration error rather than rendering an empty or successful result when the binding fails.
   - Preserve the four compiler-owned event kinds and their producer identity. Show historical unmarked-v2 read-only debt or compiler-contract lint warnings; do not present them as an acceptance failure or actual runtime cost.
...
4. Inventory `.task/cycle` in bounded read-only mode, excluding the `dashboard.md` projection being rendered and the current dashboard stage's self-referential compiler directory. Show configured file/byte/age overage, verified duplicate packet candidates, `current_stage.json` projections, and protected evidence counts; do not follow symlinks or archive/delete/apply. Missing or malformed `record_retention_policy.budgets` fails quiet with null overage.
5. Write `.task/cycle/<cycle-id>/dashboard.md` in Korean, preserving canonical step/status tokens, paths, IDs, commands, and hashes in their original language.
6. Emit the result-contract fields `task_id`, `dashboard_status`, ledger `event_count`, `valid_event_count`, explicit `current_stage_event_count`, ledger/current latest event IDs, `snapshot_status`, `retention_summary`, validation/progress verdicts, blockers, `dashboard_path`, and evidence paths. Preserve supplied task-acceptance, artifact-truth, semantic, pack-transition, historical-index, and goal-readiness verdict axes separately. Include bounded `unchanged_refs` and `unchanged_ref_count` without embedding referenced packet bodies. Link the dashboard from the ledger under `dashboard`.
...
- Do not duplicate hydrated result bodies in dashboard output. Link the compact result binding and render only bounded decision scalars, counts, blockers, and evidence refs.
