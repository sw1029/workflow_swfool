...
- `observed_output_class`: when the caller supplies loop-breaker or output-delta evidence from the `orchestrate_task_cycle progress-loop` command, prefer the observed class over self-declared progress fields. Use `material_delta` for an adapter-validated primary-artifact change. Classify `metadata_only` and repeated `terminal_record` as governance-only unless independent artifact inspection proves a material delta.
...
- Count a positive hook-backed axis only when the existing `qualitative_findings[].evidence_refs` binds an opaque hook-result id and its full digest, the packet binds the exact current `decision_artifact_ref`, and the reviewed hook value is actually used by the final decision path. Registration, import success, callable presence, invocation without accepted return semantics, and producer echo are availability or execution evidence only. Do not add a second receipt family for this proof.
- Determine hook applicability from acceptance and the canonical owner or adapter contract before inspecting invocation receipts. When an optional hook is absent, malformed, fail-quiet, or uninvoked and bounded acceptance does not require it, keep only the hook-backed axis `not_evaluated`; use `not_applicable` only for an explicit owner applicability decision, and leave bounded task acceptance unchanged. Receipt absence alone never establishes applicability.
...
- Optional-hook negative: the current body is fresh and an optional hook is registered, but the decision path does not consume its result and bounded acceptance does not require it. Expected verdict: the hook-backed axis is `not_evaluated`, no wiring defect is inferred, and bounded task acceptance remains independently decidable.
- Hook-consumption happy path: the current body and applicable hook input are exact-bound, the accepted hook output has an opaque result id plus full digest in `qualitative_findings[].evidence_refs`, and the final decision actually uses that value. Expected verdict: only the observed hook-backed axis may receive its ordinary positive review verdict.
