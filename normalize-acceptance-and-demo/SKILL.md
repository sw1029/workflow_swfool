...
5. When a criterion has scenario form "premise class -> expected terminal state", preserve it as `acceptance_scenarios` with an opaque `scenario_id`, opaque `premise_predicate_id` (or the existing bounded safe `premise_predicate` alias), `expected_terminal_state`, and optional `applicability: applicable|not_applicable`. Use adapter/caller `acceptance_scenarios(...)` when available, or normalized acceptance fields when already supplied. If no reliable predicate can be extracted, preserve a bounded safe clause and mark scenario coverage `needs_review` rather than inventing a predicate. When that clause would expose source/body/entity/locator/prompt/response-linked identifying information, store only an opaque owning-store reference plus full digest; do not quote the body. Bind this declaration to the current acceptance/task fingerprint, but do not emit an invocation receipt during normalization; `$run-task-code-and-log` owns that receipt after an actual invocation. A safe bounded clause remains directly reviewable, while a sensitive or source-linked clause remains traceable without becoming a global skill default.
...
- Do not set `premise_satisfied`, `scenario_uncovered`, or a test-count summary as scenario proof during normalization. Emit only the exact scenario declaration; the run owner emits the actual-invocation receipt and completion validation recomputes coverage from it.
