...
2. Read task and rule context.
   ...
   - When the requested doctoring changes provider, API credential, `.env`, terminal-blocker, or seal constraints, run `$orchestrate-task-cycle` `scripts/detect_gt_constraint_conflict.py --root .` on the current task and account for any conflict before writing the replacement.
