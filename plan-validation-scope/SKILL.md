...
Treat read-only planning separately from publishing a durable scope. The latter is `publish_validation_scope` in `authority.operations.json` and follows the shared [authority v2 contract](../manage-agent-authority/references/authority-v2-contract.md) with an exact scope subject and operation-bound lease.
...
1. Before governance, collect planned files or surfaces from `task.md`, acceptance, and repository policy. Run the `plan` command; an unknown planned surface conservatively selects `affected_chain` and stays visibly unfinalized.
   - Consume a supplied normalized `validation_predicate_contract` before making a dependent run eligible. Continue normally only when every applicable required satisfiability row passes. A failed row blocks the dependent run and routes the nearest producer/schema/predicate repair; a required `not_evaluated` row keeps the manifest unfinalized until its missing input is supplied. Explicit `not_applicable` rows remain excluded rather than failed.
   - For validation that consumes an existing decision artifact, always bind exact opaque subject, authoritative revision, and full digest. Bind body fingerprint, lane, cohort, and producer run only when their explicit applicability objects say `applicable`; `not_applicable` is a normal bypass. A missing or mismatched applicable binding makes only that dependent decision `not_evaluated` and plans current-subject recomputation instead of reusing a favorable projection.
...
3. Choose:
   ...
   - Treat verifier independence as both source separation and decisive-invariant separation. Disjoint paths that reuse the producer's merge, ranking, disposition, or acceptance function are coupled and require affected-chain revalidation plus provenance downgrade; disjoint inputs and a separately owned decisive invariant may follow the otherwise selected profile. Reuse the existing source-separation gate rather than creating another validation phase.
...
- Do not publish a run-eligible finalized manifest from a failed or required-unevaluated satisfiability row, a stale decision-subject binding, or source-disjoint but invariant-coupled verification.
