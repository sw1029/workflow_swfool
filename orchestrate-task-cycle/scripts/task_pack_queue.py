...
@contextmanager
def pack_mutation_lock(root: Path):
    """Serialize task-pack mutations across threads and processes."""

    directory = _require_within(pack_dir(root), root, "Task pack directory")
    directory.mkdir(parents=True, exist_ok=True)
    lock_path = _require_within(directory / ".pack-mutation.lock", directory, "Task pack mutation lock")
...
def write_content_addressed_file(path: Path, payload: bytes, label: str) -> str:
    ...
    transaction = getattr(_CONTENT_ADDRESSED_WRITE_STATE, "current", None)
    if isinstance(transaction, ContentAddressedWriteTransaction):
        transaction.register_created(path)
...
def validate_pack(data: dict[str, Any], path: Path | None = None) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(items):
        ...
        if verdict_version == 1 or any(value is not None for value in supplied_verdict_axes.values()):
            ...
            semantic_status = verdict_axis_status(supplied_verdict_axes.get("artifact_semantic_verdict"))
...
def command_apply_mutation(args: argparse.Namespace) -> int:
    root = Path(args.root).resolve()
    with pack_mutation_lock(root), content_addressed_write_transaction() as evidence_transaction:
...
def _command_apply_mutation_locked(args: argparse.Namespace, root: Path) -> int:
    ...
    if action not in {
        ...
        raise SystemExit(
            "Mutation action must be create, promote, normalize_initial_selection_provenance, insert, reorder, skip, supersede, or terminal_block."
        )
    ...
    if action == "create":
        ...
        if isinstance(initial_selection, dict):
            ...
            if origin not in {"bootstrap_initial_selection", "authorized_initial_selection"}:
                raise SystemExit("Create initial_selection requires an initial promotion origin.")
            target = next(
                (
                    item
            ...
            if target is None or not ordered or target is not ordered[0] or target.get("order") != 1:
                raise SystemExit("Create initial_selection must target the first canonical pack item.")
            if target.get("status") != "planned":
                raise SystemExit("Create initial_selection requires a planned first item.")
            task_path = bounded_workspace_file(root, task_path_value, "Create initial task_path")
            task_digest = sha256_file(task_path)
            snapshot_directory = _require_within(
                pack_dir(root) / "task_snapshots" / pack_id,
                pack_dir(root),
                "Create initial task snapshot directory",
            )
            ...
            task_snapshot_path = _require_within(
                ...
                pack_dir(root),
                "Create initial task snapshot path",
            )
            write_content_addressed_file(task_snapshot_path, task_path.read_bytes(), "Create initial task snapshot")
            supplied_receipt = initial_selection.get("initial_selection_receipt")
            if not isinstance(supplied_receipt, dict):
                raise SystemExit("Create initial_selection requires initial_selection_receipt.")
            if supplied_receipt.get("task_snapshot_ref") != rel_path(root, task_snapshot_path):
                raise SystemExit("Create initial-selection receipt references a different task snapshot.")
            if supplied_receipt.get("pack_creation_snapshot_ref") != durable_creation.get("creation_snapshot_ref"):
                raise SystemExit("Create initial-selection receipt references a different creation snapshot.")
            verified = validate_initial_selection_receipt(
                root,
                path,
                pack_data,
            ...
            target["promotion"] = {
                ...
                "mutation_evidence_paths": verify_evidence_files(
                    ...
                    "Create initial-selection evidence_paths",
                )
            ...
            final_findings = validate_pack(pack_data, path)
            if any(item.get("severity") == "block" for item in final_findings):
                raise SystemExit(
                    "Create initial-selection transaction failed final validation: "
                    + "; ".join(str(item.get("code")) for item in final_findings if item.get("severity") == "block")
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    mutate_p = sub.add_parser(
        "apply-mutation",
        description=(
            "Apply create, promote, normalize_initial_selection_provenance, insert, reorder, "
