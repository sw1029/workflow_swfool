...
from .registry import prepare_feature_symbol_registry_update
from .values import now_iso



class ResultBuilderCore(ContextBoundStage):
    name = "result"

    def run(self, context: AnalysisContext) -> None:
        self.bind(context)
        self._build_terminal_blocker_candidate()
        context.result = self._build_result()
...
class ResultBuilderStage(ResultBuilderCore):
    """Supply final gate summaries used by result rendering."""
...
class ResultBuilderMixin(ResultBuilderStage):
    """Compatibility name for the result stage strategy."""
