...
from .evidence import collect_sealed_families
from .registry import terminal_history_matches
from .values import boolish



class ProgressAggregationCore(ContextBoundStage):
    name = "aggregation"
    ...
    def _derive_repetition_lists(self) -> None:
        ...
        def reached(count: int) -> bool:
            return recurrence is not None and count >= recurrence
...
class ProgressAggregationStage(ProgressAggregationCore):
    """Aggregate derived metrics after the shared counting phase."""
...
class ProgressAggregationMixin(ProgressAggregationStage):
    """Compatibility name for the aggregation stage strategy."""
