...
def explicit_identity_object(value: object) -> dict[str, Any] | None:
    """Extract the exact identity object from one known packet envelope boundary."""
    if not isinstance(value, dict):
        return None
    nested = value.get("decision_identity")
    if isinstance(nested, dict):
        return nested
    for field in ("decision_artifact_ref", "selected_artifact_ref", "artifact_ref"):
        nested = explicit_identity_object(value.get(field))
...
def normalized_legacy_identity(identity: object) -> dict[str, Any]:
    ...
    return {
        **identity,
        ...
        "body_projection_fingerprint": projection.dimension_values.get(
            "body_fingerprint"
        ),
        "production_lane_identity": projection.dimension_values.get("production_lane"),
        "verification_input_ids": projection.dimension_values.get("cohort"),
        "producer_run_id": projection.dimension_values.get("producer_run"),
    }
