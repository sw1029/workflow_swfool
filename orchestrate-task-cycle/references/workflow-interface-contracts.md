...
| `result_contract.py` | `--target <target>`, `--mode warn|block`, result JSON; stable facade over `result_contract_lib` rule registry | Contract findings, ledger-envelope readiness, Part J transition warnings for scenario coverage, argv provenance, blocker actionability, stochastic feasibility, and first-fire credit ownership, plus Part K warnings/blocks for stale expectations, parity/adoption routing, resolution downgrade, report-key divergence, and warn-only matching duplicate report-key schema debt | Orchestrator before advancing stages; focused consumers may instantiate a target rule through `RuleContext`/`RuleRegistry` |
