...
| Derive packet | `$derive-improvement-task` | Schema reconciliation, final index, commit, dashboard, report | Re-verified predecessor `cycle_finalization_receipt` and its six-axis authoritative projection, issue provenance, selected next task or terminal/deferred disposition, canonical task-pack coherence/mutation receipt, promotion origin, blockers, and evidence paths. Initial/bootstrap or genuinely standalone repair may record reasoned `not_applicable`; normal post-validation derivation cannot substitute a pre-validation anti-loop handoff. It cannot override a finalized hard stop or retry an unrelated verdict axis. |
