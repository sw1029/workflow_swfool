...
| Anti-loop progress gate | `$audit-cycle-loopback` | Derive, validation, dashboard, report | Family/root keys, semantic signature, exact decision artifact ref, compatibility-filtered gate IDs, full required consumer invocation receipts, axis-scoped verification provenance, progress booleans, terminal outcome fields, effective dispositions, hard-stop state, and a bounded hash-bound derive handoff. The handoff carries ref/hash plus authoritative scalars; it does not embed the packet body. |
...
| Task-pack packet | `scripts/task_pack_queue.py` and `$derive-improvement-task` | Derive, index, validation, report, commit | Pack ID/path/status, current item, canonical before/after coherence receipt, mutation plan, promotion origin, Markdown render, terminal blocker state, and selected disposition. Preserve task acceptance, artifact truth/semantics, pack transition, historical index, and goal readiness as separate verdict axes. |
...
| Validation result | `$validate-task-completion` | Issue, schema, derive, post-derive index, commit, dashboard, report | Current `task_id`, legacy validation/progress summaries, six evidence-bound verdict axes, exact decision artifact identity, compatible gate set, required consumer invocation receipts, axis-scoped verification provenance, blockers, and evidence paths. A pack/index transition failure must not erase a passed implementation acceptance; semantic failure must not become goal progress. |
...
| Derive packet | `$derive-improvement-task` | Schema reconciliation, final index, commit, dashboard, report | Validated current-task verdict axes, issue provenance, hash-bound anti-loop handoff or reasoned initial/standalone `not_applicable`, selected next task or terminal/deferred disposition, canonical task-pack coherence/mutation receipt, promotion origin, blockers, and evidence paths. It cannot override a handoff hard stop or retry an unrelated verdict axis. |
...
G-COV producers and consumers use `quality_delta_policy` as the sole metric-key/alias contract. The normalized shape is `keys: [string]`, `aliases: {string: [string]}`, and `supplied: bool`. `output_delta_contract.py` accepts it from the repository contract or provider payload; loopback accepts it from the domain adapter hook; progress-loop detection accepts it from the packet. Without it, all three emit or preserve `not_evaluated` rather than inventing metric axes.
...
| `mode_profile.py` | `validate|resolve`, tracked profile registry, optional reducing override, explicit activation source | Closed non-GT mode resolution with capture/consume/reaction effects and exact derived-index repair scope | Coordinator before optional session observation behavior |
