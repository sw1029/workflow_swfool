...
- Treat pending long-running statuses (`launching`, `running`, `completed_pending_validation`, `stale`, `not_running`) as incompatible with final-output-dependent review, loopback, ordinary derivation, issue closure, `validation_verdict: passed`, `progress_verdict: advanced`, and `complete_verified`. Select monitor, harvest, finalize, terminal blocker, or user escalation for the same `run_id`.
