...
Under `모델/effort 라우팅`, report policy/profile/tier, model reference and configuration status, reason codes, signals, violations, and distinguish `enforced`, `prompt_only`, and `inherited_unverified`. Do not imply the already-running coordinator or any child used a configured model when the runtime did not expose resolved binding and enforceable routing evidence. Report deterministic-only phases compactly and surface any tier/model/effort deviation or forbidden delegated `ultra`. A direction-profile route must identify Tier 5 final-direction ownership and signal evidence; a bounded direction-profile/max route must also identify `prior_tier5_evidence`, `max_escalation_reason`, and one-agent enforcement.
...
When an approval UX review projection is present, add a compact `승인 UX` entry
under `추가 참고`: reviewed requests, reused states, signed-mode children, system
recoveries, deduplicated waits, remaining prompts, batched prompt groups, and reviewer
agent count. Report only observed transitions and consolidations; do not estimate
counterfactual prompts avoided. Never include config contents, session text, approval
projection bodies, credentials, tokens, or secrets.
