...
- L5 cycle-scale reachability: normalize `acceptance_scale(...)` and `throughput_evidence(...)` into a pure content-bound calculation. Matching units plus positive required scale, per-cycle throughput, and cycle cap are mandatory. Throughput confidence upper-bound capacity below the target proves `unreachable`; lower-bound capacity at/above target proves `reachable`; missing/incomparable inputs or a crossing interval are fail-quiet `not_evaluated`/`indeterminate`. An unreachable gate carries stable scale/throughput IDs and `cycle_reachability_sha256`. Before launch, derive may select only a digest-bound long-run launch with residual acceptance and harvest plan, throughput improvement, explicit residual descope, terminal blocker, or user escalation. After launch, monitor/harvest/finalize selection must bind the exact pending `run_id`, digest, and `harvest_plan_id`; a small smoke repeat or phase-incompatible route is not goal-productive.
...
Use the closed v2 authority packet from [authority-boundary-contract.md](authority-boundary-contract.md). Legacy `authority_classification` rows are unverified diagnostics and cannot support terminal, wait, or escalation. Keep local resolution, external input, and risk/cost consent out of the authority axis.
