...
- For the initial all-three-absent `grant_authority` projection only, run `selected-successor ... compile-approval-batch`, then the ordinary signed root-plan/materialization flow, and finally `selected-successor ... resume-authority` with that exact projection and materialization receipt. Keep every plan, signed decision, and continuation time strictly later than the original request-compilation time; never approve the projection directly or recompile it to advance the clock. Mixed grant state fails closed. The topology plan does not authorize the fresh-cycle dispatch/run plan.
