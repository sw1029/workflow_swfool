...
- Use protocol-v2 with preparation-v3 for new cycles. Preserve initialized protocol/preparation markers and keep v1/v2 artifacts readable and recoverable without rewriting them.

The ledger CLI and direct API initialize new cycles with compiler protocol v2 and preparation schema v3 by default. An existing cycle retains its initialized protocol/preparation markers; metadata without a protocol field remains v1 and must not be upgraded implicitly.
...
Compiled results are stored once as canonical packet CAS bytes. Every compiler CAS write uses its actual immutable-link receipt: add new/reused bytes and actual new-file counts from context, work-order or machine-input, preparation, and result writes instead of replacing earlier counters. For schema-v3 preparation publication, let the compiler write and verify preparation-scoped origin intents before CAS links, then derive the aggregate origin receipt from those intents; this preserves the original classification across a crash before aggregate receipt creation. Keep per-attempt CAS counters outside preparation identity and durable preparation bytes so a replay reuses the same preparation CAS; a retry after a result write but before ledger append records the result bytes as reused. The authoritative ledger row is a compact format-v2 binding plus bounded scalars; public readers hydrate and reverify the exact bytes, while raw readers expose the compact row for integrity/profiling. Exact replay must not append another ledger event. Never manually duplicate a result or packet body into `stage.jsonl` or `current_stage.json`.
...
Initialize ledger storage in `initialization.json`; this is not a stage event. Append `context` as the first canonical ledger row. Before each major subskill call, render the applicable packet with `python3 -P -m orchestrate_task_cycle packet`, validate accumulated ordering state with `python3 -P -m orchestrate_task_cycle transition --stage ...`, and pass the current target routing packet separately with `--routing-json ...`. Validate important subskill results with `python3 -P -m orchestrate_task_cycle result-contract`, supplying `--context` whenever pending long-run state is in scope, using `warn` by default and `block` for acceptance task-revision provenance, final report, running-state, issue lifecycle provenance, candidate deletion, task-pack promotion provenance, and commit gates.
