...
- For a normal-cycle selected successor, run `selection-decision-receipt ... pipeline --trigger-kind normal_cycle` with the exact finalization, schema-pre-derive, current-task, task-index, and publication-head bindings. Then use `selection-publication ... prepare-intent` and the prospective task-state/publication transaction in [selection-reentry-and-publication.md](references/selection-reentry-and-publication.md): task-state becomes pending, selection publication CAS-updates `task.md` last, task-state settles from that committed receipt, and only then may selection be consumed or a predecessor terminal-wait baseline be retired. Do not author target arrays, Base64 payloads, lineage IDs, digests, or receipts in coordinator context.
