...
- Run `selected-successor ... prepare-authority-context` before `prepare-authority`. Supply the exact bundle, actor rank, decision axes, evidence, and complete actual session/goal ceilings; consume only its producer-CAS context bindings. Pass one closed present/absent grant choice for each topology action. Only the canonical evaluator may return `allowed`; neither command creates or infers a source approval or grant, actor identity, or semantic scope. A genuine non-allowed result publishes one compact projection and zero lifecycle/topology effects, while an absent declaration that evaluates allowed is a conflict. Apply the full location, replay, and legacy rules in [selection-reentry-and-publication.md](references/selection-reentry-and-publication.md).
- For the initial all-three-absent `grant_authority` projection only, run `selected-successor ... compile-approval-batch`, then the ordinary signed root-plan/materialization flow, and finally `selected-successor ... resume-authority` with that exact projection and materialization receipt. An eligible `authority_interaction_mode` may materialize one exact mode-child per topology request; `resume-authority` must reopen its child materialization and request binding. Keep every plan, signed decision, and continuation time strictly later than the original request-compilation time; never approve the projection directly or recompile it to advance the clock. Mixed grant state fails closed. The topology plan does not authorize the fresh-cycle dispatch/run plan; that fresh request requires a distinct exact child.
