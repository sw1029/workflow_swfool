...
description: "Run the governed repository task workflow cycle: ensure `task.md`, resolve `.agent_goal/agent_authority.md`, consume repo-local `.codex/skills` adapters as compact non-GT packets, plan/build validation assets through `$build-validation-set-with-agents`, route implementation through `$task-md-agent-governance`, audit code structure before execution, log execution, run single-agent qualitative review, audit loopback progress, refresh schemas, derive the next task/task-pack transaction with fixed `xhigh`, optionally schedule repo-local adapter creation/update through `$skill-creator`, preserve one active `task.md`, enforce blocker/input-delta loop breakers and terminal state, index artifacts with fixed `medium`, validate progress axes, track issues, commit coherent partial or complete changes with fixed `low`, report in Korean, and keep the orchestrator summary-first."
---
...
- [anti-loop-progress-gates.md](references/anti-loop-progress-gates.md): loopback progress gates, output-delta policy, adapter mandate, cumulative goal-distance stalls, acceptance reachability, oracle/metric validity, gate satisfiability, terminal quiescence, terminal escalation, and no-overclaim boundaries.
