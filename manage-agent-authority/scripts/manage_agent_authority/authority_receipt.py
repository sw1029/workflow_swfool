...
def resolve_workspace_file(root: Path, value: Any, label: str) -> Path:
    ...
    if not path.is_file():
        raise SystemExit(f"{label} does not identify an existing regular file: {raw.as_posix()}")
    return path
...
def validate_receipt(
    ...
    if receipt.get("schema_version") != 1 or receipt.get("receipt_kind") != "operation_authority":
        raise SystemExit("Authority receipt requires schema_version=1 and receipt_kind=operation_authority.")
    if not str(receipt.get("receipt_id") or "").strip():
        raise SystemExit("Authority receipt requires a stable receipt_id.")
    if receipt.get("operation") not in OPERATIONS or receipt.get("decision") != "allowed":
        raise SystemExit("Authority receipt operation is unsupported or decision is not allowed.")
    ...
    selected = parse_time(selected_at, "selected_at") if selected_at is not None else None
    sensitive = forbidden_key_paths(receipt)
    if sensitive:
        raise SystemExit(f"Authority receipt contains forbidden sensitive/body keys: {', '.join(sensitive)}")
    ...
    missing = [field for field in required_subject if subject.get(field) in (None, "")]
    if missing or subject.get("initial_order") != 1:
        raise SystemExit(f"Authority receipt subject is incomplete or not first-order: {', '.join(missing)}")
    required_sha(subject.get("pack_creation_snapshot_sha256"), "subject.pack_creation_snapshot_sha256")
    ...
    if expected_subject is not None and subject != expected_subject:
        raise SystemExit("Authority receipt subject does not match the expected operation subject.")

    basis = receipt.get("authority_basis")
    if not isinstance(basis, dict):
        raise SystemExit("Authority receipt authority_basis must be an object.")
    policy_path = resolve_workspace_file(root, basis.get("policy_ref"), "authority_basis.policy_ref")
    source_path = resolve_workspace_file(root, basis.get("source_evidence_ref"), "authority_basis.source_evidence_ref")
    if sha256_file(policy_path) != required_sha(basis.get("policy_sha256"), "authority_basis.policy_sha256"):
        raise SystemExit("Authority policy SHA-256 does not match the receipt.")
    if sha256_file(source_path) != required_sha(basis.get("source_evidence_sha256"), "authority_basis.source_evidence_sha256"):
        raise SystemExit("Authority source-evidence SHA-256 does not match the receipt.")
    ...
    if source_kind not in AUTHORITY_SOURCE_KINDS or not source_id:
    ...
    if temporality == "current_ratification":
        if source_kind != "explicit_current_user_instruction":
            raise SystemExit("Current ratification requires explicit current user instruction evidence.")
        if historical.get("historical_selection_authority_status") != "unverifiable_before_ratification":
            raise SystemExit("Current ratification must preserve unverified historical selection authority.")
        if historical.get("historical_authority_verdict") != "partial" or historical.get("retroactive_claim_allowed") is not False:
            raise SystemExit("Current ratification cannot claim a historical pass or retroactive authority.")
        if selected is not None and effective <= selected:
            raise SystemExit("Current ratification must be effective after the original selection.")
    elif temporality == "contemporaneous_selection_authority":
        if source_kind not in CONTEMPORANEOUS_SOURCE_KINDS:
            raise SystemExit("Contemporaneous authority requires a supported contemporaneous source.")
        if (
            ...
            raise SystemExit("Contemporaneous authority requires a verified historical pass without retroactive claims.")
        if selected is not None and effective > selected:
            raise SystemExit("Contemporaneous authority must be effective by the original selection.")
        ...
        if (
            ...
            raise SystemExit("Retrospective assessment may only normalize from a contemporaneous authority record.")
        if (
            historical.get("historical_selection_authority_status") != "verified"
            or historical.get("historical_authority_verdict") != "pass"
            or historical.get("retroactive_claim_allowed") is not False
        ):
            raise SystemExit("Retrospective assessment requires verified immutable historical authority.")
        if selected is not None and effective > selected:
            raise SystemExit("Retrospective evidence must predate or equal the original selection.")

    effects = set(str(item) for item in receipt.get("allowed_effects") or [])
    if receipt.get("operation") == "task_pack.normalize_initial_selection" and effects != NORMALIZATION_ALLOWED_EFFECTS:
        raise SystemExit("Task-pack normalization receipt effects must be provenance-only.")
    if receipt.get("operation") == "task_pack.initial_selection" and effects != INITIAL_SELECTION_ALLOWED_EFFECTS:
        raise SystemExit("Task-pack initial-selection receipt effects must be first-promotion only.")
    return {
        "status": "valid",
        "receipt_id": receipt.get("receipt_id"),
        "operation": receipt.get("operation"),
        "basis_temporality": temporality,
        "temporality_binding_status": "verified" if selected is not None else "consumer_required",
        "subject": subject,
...
def command_issue(args: argparse.Namespace) -> int:
    ...
    policy = resolve_workspace_file(root, basis.get("policy_ref"), "authority_basis.policy_ref")
    source = resolve_workspace_file(root, basis.get("source_evidence_ref"), "authority_basis.source_evidence_ref")
    ...
    try:
        ...
        raise SystemExit("Receipt output must stay under .task/authority_receipts/.") from exc
    if output.suffix != ".json":
        raise SystemExit("Receipt output must be JSON.")
    if output.exists():
        existing = json.loads(output.read_text(encoding="utf-8"))
        if existing != receipt:
            raise SystemExit("A conflicting authority receipt already exists at the output path.")
    else:
        write_json_atomic(output, receipt)
    result = {**validated, "receipt_ref": output.relative_to(root).as_posix(), "receipt_sha256": sha256_file(output)}
...
def command_validate(args: argparse.Namespace) -> int:
    ...
    if args.receipt_sha256 and sha256_file(path) != required_sha(args.receipt_sha256, "receipt_sha256"):
        raise SystemExit("Authority receipt SHA-256 does not match.")
    receipt = json.loads(path.read_text(encoding="utf-8"))
    expected = load_json_value(args.expected_subject_json) if args.expected_subject_json else None
    result = validate_receipt(root, receipt, expected, getattr(args, "selected_at", None))
    result.update({"receipt_ref": path.relative_to(root).as_posix(), "receipt_sha256": sha256_file(path)})
    json.dump(result, sys.stdout, ensure_ascii=False, indent=2, sort_keys=True)
    sys.stdout.write("\n")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Issue or validate bounded operation authority receipts.")
    parser.add_argument("--root", default=".")
    subparsers = parser.add_subparsers(dest="command", required=True)
    issue = subparsers.add_parser("issue")
    issue.add_argument("--plan", required=True, help="Receipt JSON or a JSON file path.")
