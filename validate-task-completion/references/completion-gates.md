...
- [Governed final candidate contract](#governed-final-candidate-contract)
- [Gate status values](#gate-status-values)
- [Progress verdict values](#progress-verdict-values)
...
- `durable_state_candidate.mode: complete_projection | typed_operations`, with complete payloads rather than digest-only mutations. `complete_projection` is valid only as the helper-built exact `no_durable_state_change` receipt.

Typed-operation candidates use the exact v2 producer contract: `contract_version: 2`, opaque `producer`, outer `attempt_identity`, ordered operation IDs and expected target revisions, `operation_set_digest`, and `candidate_sha256`. Each operation binds deterministic operation/idempotency IDs, target kind/ref, registered operation kind, expected target revision, the same attempt, dependencies, payload schema, complete payload and digest, and recovery policy. Reject v1, anonymous, digest-only, mismatched-alias, reordered, stale-target, or empty typed candidates. An empty durable delta must instead use the exact same-attempt no-change reason/evidence receipt.
...
Reviewer availability limits only the review-backed axes. A deterministic bounded task may keep `task_acceptance_verdict: pass` while `artifact_semantic_verdict: not_evaluated` and `goal_readiness_verdict: blocked`; this is local completion evidence, not global semantic progress. When the current task index consumes the verified finalization, it may preserve that task as completed, current history and non-executable while residual/global successor selection remains separate.

Map outcome claims from the existing axes rather than adding another truth field: global semantic movement requires exact-bound semantic and goal-readiness pass with independent, axis-complete evidence; stable-root reduction may reset only the matching root when loopback supplies independently verified or explicitly self-grounded residual reduction; bounded task acceptance may close only the task; terminal claims require exact authority/local/external exhaustion evidence; retained safety/governance evidence remains bounded to those roles.

When optional scoped fields are present, recompute them under the orchestrator's [scoped-progress and retained-change contract](../../orchestrate-task-cycle/references/workflow-interface-contracts.md#scoped-progress-and-retained-change). In particular, `work_intent.expected_scope` is not evidence; `retained_change_classification` must agree with content-bound per-file change receipts and role-bound before/after receipts; caller changed-path lists alone are not evidence; and tests/schema/verifier/lifecycle-only changes cannot support semantic or global-reset claims. Preserve a valid `completed_local` task with `review_axis: unavailable`, `global_readiness: blocked`, and `successor_state: terminal_wait`; do not reactivate it as executable merely because no successor was derived.

The orchestrator finalizer validates candidate hashes, privacy, dependencies, target revision CAS, and the complete expected-previous tuple; assigns same-attempt revision/supersession; reload-verifies one content-addressed immutable snapshot; and only then switches the compare-and-swap current pointer. It reloads the published state and binds predecessor projection, operation set, target revisions, and post-write aggregate digest into the committed receipt. A finalization error leaves the prior pointer authoritative. A conflict remains a non-authoritative pending record until a content-bound merge/retirement resolution is itself committed and loader-verified. Consumers must verify the current receipt and echo its token, attempt/revision, projection ID/digest, and receipt hash before deriving, promoting, reporting, or closing.
...
| Acceptance scenario | Every applicable scenario-shaped criterion has a valid receipt bound to its scenario/predicate IDs, positive premise-satisfying count, invocation descriptor and digest/body-free CLI argv, input revisions, result digest, run/evidence refs, and matching expected/observed terminal state. Coverage and inversion are recomputed from receipts. | Scenario receipt is missing, malformed, premise-unsatisfied, not evaluated, or needs validation-set planning; caller booleans do not upgrade it. | A valid premise-satisfying invocation observes/asserts the opposite terminal state, or completion relies on descriptions/tests that never invoke the premise. |
...
| Verification source separation | `independently_verified_fields` use disjoint inputs and a separately owned decisive invariant, or the adapter marks the affected structural axis `self_grounded=true`. | Source or decisive-invariant separation is missing/overlapping but affected fields are downgraded to attested and residual verification remains open. | Completion, progress, high-water reset, or `goal_productive` relies on fields whose inputs or decisive merge/ranking/disposition invariant remains coupled. |
...
| Lane identity | Verifier/review/metric passes consumed for current-lane capability, adoption, comparison, baseline, next-rung, close, or progress match `current_decision_lane`, or lane hooks are absent and no caller requires lane binding. | Lane hooks are missing, or stale-lane evidence is preserved with current-lane residual work open. | `pass_on_stale_lane=true` is consumed as current-lane evidence. |
| Decision freshness | Exact decision subject is current through content-bound implementation→deliverable compatibility and deliverable→review relation receipts; IDs from different revision namespaces need not equal. Applicable producer-run work also has a content-bound current measurement receipt with run id and output fingerprint; a permitted no-impact receipt binds the same subject/revision and named predicate. | `implementation_ahead_of_artifact` keeps a valid older implementation→deliverable relation and remains metadata/governance or producer-refresh work; `artifact_ahead_of_review` keeps a valid older deliverable→review relation, preserves artifact production, and leaves review-backed readiness open. | Equal-looking cross-namespace IDs substitute for relation receipts; a relation endpoint/digest is stale or tampered; an arbitrary truthy no-impact flag, missing applicable run receipt, or `no_domain_artifact` state is consumed as semantic progress, adoption, high-water movement, or review-backed readiness. |
| Gating-axis producer | A starved gating axis has producer-supply evidence, or the task explicitly descopes, terminal-blocks, or escalates. | Producer supply remains open and verifier-like work is classified as non-progress. | Another verifier, guard, consistency check, or report is consumed as goal-productive while `axis_starved_by_missing_producer=true`. |
| Portfolio quota | Restrictive verifier-like portfolio quota was honored, recovered, or routed to producer/envelope/long-run/descope/terminal/escalation work. | Quota evidence is warn-only or residual producer-like work remains open. | A verifier/guard/report/metadata item is claimed goal-productive while restrictive `portfolio_quota_exceeded=true` remains active. |
| Cycle reachability | Cycle-unreachable scale targets are met by a content-bound matching harvest receipt or a same-scale fresh-throughput recalculation that proves reachable. Explicit residual descope, terminal blocker, and escalation preserve rather than satisfy the original target. | Long-run launch/monitor evidence exists but harvest validation remains open. | The original target is consumed from smoke, launch, startup, heartbeat, `False`/truthy flags, a mismatched receipt, or an unbound throughput claim while `unreachable_within_cycle=true`. |
| Metric basis | Claimed metric basis classes are derivable from consumed inputs, or affected metrics are downgraded and excluded from independent progress consumption. | Basis-overclaimed metrics are preserved as trace or downgraded evidence with residual measurement open. | `basis_overclaim=true` metrics support completion, high-water movement, or `advanced` progress as independently verified evidence. |
| Surface field review | Locator-backed qualitative review covers all adapter-supplied producer-written surface field classes and has no unresolved blocking defect counts for consumed fields. | Field-class map is missing fail-quiet, or nonzero counts remain with residual producer/field repair. | A review pass is consumed for affected field classes despite unresolved nonzero `surface_field_defect_matrix` counts. |
...
| Terminal authority axes | Every terminal-relevant residual separately proves authority, local-resolution, external-dependency, and risk/cost-confirmation status. Local work routes in-scope repair, waiting state routes monitor/harvest, and verified authority/confirmation debt routes escalation. | Any axis is `unverified`, so bounded classification/probe work remains open and no terminal claim is consumed. | Task/pack exhaustion or a single residual label is used to claim `terminal_blocked`, waiting is treated as missing user input, or local resolution/confirmation debt is hidden. |
...
Return `partial` when scenario-shaped acceptance lacks a valid structured premise receipt. Return `failed` if a positive close relies on caller booleans, or if a valid premise-satisfying invocation observes/asserts the opposite terminal state.
...
Return `partial` when a pass is on a stale lane and current-lane rerun or residual scope remains open. Return `failed` if that pass is consumed as current-lane capability, adoption, comparison-winner, baseline, next-rung, close, or progress evidence.

Return `partial` when a decision update is only metadata revision and fresh measurement remains open. Return `failed` if stale measurement artifacts are consumed as measurement progress, adoption, or high-water movement without fresh run/no-impact proof.

Return `partial` when a gating axis is starved by missing producer supply and producer work remains open. Return `failed` if another verifier-like item is consumed as goal-productive for that starved axis.

Return `partial` when restrictive portfolio quota leaves producer/envelope/long-run/descope work open. Return `failed` if verifier-like work is validated as goal-productive while the restriction remains active.

Return `partial` when cycle-unreachable scale requires long-run harvest, throughput improvement, descope, terminal blocker, or escalation. Return `failed` if the original target is consumed from smoke, launch, startup, or heartbeat evidence alone.

Return `partial` when metric basis is overclaimed and downgraded evidence remains open. Return `failed` if basis-overclaimed metrics are consumed as independently verified completion or progress.

Return `partial` when surface-field defect counts remain with residual repair or descope open. Return `failed` if review pass is consumed for affected field classes despite unresolved nonzero defect counts.
