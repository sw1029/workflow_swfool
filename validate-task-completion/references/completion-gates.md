...
| Structure effect | Behavior-preserving refactor/consolidation moved an adapter-supplied structure high-water target enough to satisfy the task objective, such as lower shard/coupling/duplicate/depth/fan-out burden, better reuse ratio, or lower max LOC. | Refactor code and tests are useful but structure high-water movement is missing, weak, producer-local only, or deferred. | The task claims structural reduction while target structure metrics are flat or worse, or file-count growth/mechanical splitting/token avoidance is the only claimed progress. |
