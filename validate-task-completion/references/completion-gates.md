...
| Decision freshness | Decision/adoption/reclassification updates after upstream production-contract changes use a fresh current-lane run id, or prove the change cannot affect the measured axis. | The update is useful metadata/governance work but fresh measurement remains open. | `decision_metadata_revision=true` or stale measurement artifacts are consumed as measurement progress, adoption, or high-water movement. |
...
| Cycle reachability | Cycle-unreachable scale targets are met by long-run harvest validation, throughput improvement, explicit residual descope, terminal blocker, or user escalation. | Long-run launch/monitor evidence exists but harvest validation remains open. | The original scale target is consumed from a small smoke, launch-only handoff, startup, or heartbeat while `unreachable_within_cycle=true`. |
