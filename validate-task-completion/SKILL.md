...
Receipt-backed validation materialization is the non-authoritative `publish_validation_projection` operation declared in `authority.operations.json`. Its `none` mechanism never authorizes implementation, task advancement, finalization, or commit; those remain separate exact operations under the shared [authority v2 contract](../manage-agent-authority/references/authority-v2-contract.md).
...
1. Collect local evidence.
   ...
   - When retained change affects the verdict, require the execution-start `retained_change_baseline` and recompute the after comparison from current bytes with `python3 -m orchestrate_task_cycle.retained_change_evidence compare --root . --baseline-json <cycle-baseline.json>`. Consume its exact `retained_change_evidence`; reject a task-written file list, commit message, status label, or row whose before/after relation does not match that comparison. Missing or drifted baseline evidence makes retained-change classification `not_evaluated`, not unchanged or progress.
...
5. Run repository audit.
   ...
   - If multi-agent inspection is unavailable, state that limitation and keep review-backed artifact-semantic or goal-readiness axes `partial`/`not_evaluated`. Do not downgrade independently established deterministic bounded `task_acceptance_verdict: pass`; the six axes may legitimately record local acceptance pass with review unavailable and global readiness blocked. An available, axis-complete review follows the ordinary complete path.
   - When scoped progress is supplied, apply [the shared task/root/global and retained-change contract](../orchestrate-task-cycle/references/workflow-interface-contracts.md#scoped-progress-and-retained-change). Recompute the retrospective class from actual retained paths/digest pairs and exact observations. A prospective global intent cannot upgrade task-local evidence; root and global stall resets have distinct proof floors; incomplete/conflicting goal axes keep global readiness blocked or not evaluated. A bounded accepted task may close as immutable `completed_local` with reviewer unavailable and `successor_state: terminal_wait`.
...
10. Evaluate acceptance provenance, convention conformance, and structure effect.
   ...
   - When normalized acceptance or validation-set evidence provides `acceptance_scenarios`, recompute coverage from the structured `premise_receipts` rows (or the structured `scenario_coverage` alias). Require exact scenario/predicate binding, `premise_evaluation_status=pass`, integer `premise_satisfying_item_count>0`, invocation kind and opaque descriptor, ordered typed body-free invocation components, a canonical digest recomputed over scenario/predicate/kind/descriptor/components/input revisions, non-empty input revisions, full result digest, expected/observed terminal states, and run/evidence references. Reject raw paths/locators/entities/credentials/bodies, untyped CLI values, argv/component disagreement, and arbitrary well-shaped digests. Do not trust caller `premise_satisfied`, `scenario_uncovered`, or `acceptance_inversion` booleans without the receipt. If no valid receipt satisfies the premise and expected terminal state, set `scenario_uncovered=true` and return `partial`. If a valid premise-satisfying invocation observes the opposite state, set `acceptance_inversion=true`, return `partial`, and route code/contract repair regardless of green tests. An acceptance scenario explicitly marked `applicability: not_applicable` preserves the ordinary non-applicable path.
   ...
   - When evidence provenance reports `independently_verified_fields`, require disjoint verification/producer inputs and different bounded producer/verifier invariant-owner IDs with `invariant_separation_status=independent`. Different files or wrapper functions alone are insufficient. If either separation is missing/overlapping/coupled/unknown, treat affected fields as attested and do not use them for semantic `complete` or `advanced`. Preserve `self_grounded` only for an explicit root-local structural axis, never global or semantic evidence.
   - Evaluate exact decision-subject identity and current implementation/artifact/review freshness before scenario, metric, or favorable report consumption. An applicable subject/revision/body/lane/cohort mismatch makes the dependent gate `not_evaluated`; implementation ahead of artifact requires regeneration, artifact ahead of review limits only review-backed claims, and an all-current exact chain follows normal completion.
   ...
   - When evidence reports `unreachable_within_cycle=true`, do not complete the original scale acceptance from a small smoke, launch-only handoff, heartbeat, or a truthy completion flag. Require either (a) a content-bound harvest receipt matching the original gate digest, run, harvest plan, scale/residual IDs, unit, required scale, predicate IDs, and output fingerprint, or (b) a content-bound recomputed `reachable` gate for the same scale ID with fresh throughput evidence. Explicit descope with residual scope, terminal blocker, or user escalation remains a non-success disposition for the original target.
...
11. Decide the verdict.
   ...
   - If independently verified evidence lacks disjoint inputs or distinct decisive-invariant owners, `complete`/`advanced` is invalid for the affected metric. Root-local structural `self_grounded` may support only that local consistency axis.
...
Include an `Acceptance scenario` gate whenever `acceptance_scenarios`, `premise_receipts`, `scenario_coverage`, `scenario_uncovered`, or `acceptance_inversion` fields are present.
...
- Do not complete or advance work from `independently_verified_fields` when inputs are missing/overlapping, invariant owners are missing/shared, or fields were downgraded to attested. Do not use `self_grounded` beyond an explicit root-local structural consistency axis.
...
- Do not complete scenario-shaped acceptance from tests that never satisfy the scenario premise, from a caller boolean, or from a receipt missing invocation, revision, result, or run/evidence binding. Do not treat green tests as evidence against recomputed `acceptance_inversion`.
