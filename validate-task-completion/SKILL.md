...
- `target_metric_delta(acceptance_spec, prev_artifact, curr_artifact, **context) -> dict`: optional S7 helper returning `metric`, `before`, `after`, and `moved`. Use it immediately before completing a measurable target. If `moved=false` and the current artifact only adds measurement/proxy/observed fields for that metric, return `partial` or `failed` with `measurement_without_movement` and preserve a follow-up for real target-metric movement. If absent or malformed, fail quiet to legacy completion behavior and emit `movement_unverified`; do not infer metric meaning, measurement-field classes, or thresholds locally.

Do not define identifier surfaces, meaning floors, regeneration commands, storage policy thresholds, privacy leak equivalence, proxy populations, new-behavior samples, sample thresholds, producer-execution meaning, input-generation classes, hook registries, target metrics, measurement-only field classes, or hook provenance labels in this skill. If hooks are absent, fail quiet to legacy validation behavior unless a caller acceptance contract explicitly requires the missing gate. Q3 fail-quiet applies only when an artifact does not claim hook-produced provenance; a false hook label is fail-close when the registry is available.
...
10. Evaluate acceptance provenance, convention conformance, and structure effect.
   ...
   - When normalized acceptance or caller packets supply S7 `target_metric_delta`, require real movement of the acceptance-owned target metric before completing the target. A measurement matrix, proxy status, observed field, or predicted after-value is not completion evidence when `moved=false`; set `measurement_without_movement` and preserve a derive follow-up for the real movement. If the hook is absent or malformed, keep existing completion behavior, emit `movement_unverified`, and do not infer target metrics or measurement-only fields locally.
...
11. Decide the verdict.
   ...
   - If S7 `target_metric_delta` reports `moved=false` for the measurable acceptance target and the current artifact only added metric-measurement, proxy, or observed fields, `complete` is invalid for that target. Missing S7 hook evidence is `movement_unverified` and warning/debt only unless another acceptance contract requires movement verification.
...
Include a `Target metric movement` gate whenever `target_metric_delta`, `movement_unverified`, or `measurement_without_movement` fields are present.
...
- Do not complete measurable target work from measurement-only artifacts when S7 `target_metric_delta` reports no real target-metric movement. Missing hook evidence is `movement_unverified`, not a hard failure and not a pass.
