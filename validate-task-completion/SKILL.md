...
- `target_metric_delta(acceptance_spec, prev_artifact, curr_artifact, **context) -> dict`: optional S7/S11 helper returning `metric`, `before`, `after`, `moved`, and `artifact_class`. Use it immediately before completing a measurable target. If `moved=false` and the current artifact only adds measurement/proxy/observed fields for that metric, return `partial` or `failed` with `measurement_without_movement` and preserve a follow-up for real target-metric movement. If `moved=true` but `artifact_class` is `fixture` or `unit_scalar` and the acceptance/caller contract requires movement on a non-fixture artifact class, return `partial`, emit `movement_on_fixture_only=true`, and preserve a required follow-up for movement on the required artifact class. If the hook is absent or malformed, fail quiet to legacy completion behavior and emit `movement_unverified`; if the hook omits `artifact_class`, keep legacy S7 behavior and emit `artifact_class_unverified`. S11 is only the artifact-class extension of S7 and must not be double-counted as S9 artifact/gate compatibility.
